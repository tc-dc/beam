...
class WriteBundlesToFiles<DestinationT, ElementT>
  ...
  private transient Map<DestinationT, AbstractRowWriter<ElementT>> writers;
  ...
  public void processElement(
    ...
    if (writer.getByteSize() > maxFileSize) {
      ...
      writer.close();
      AbstractRowWriter.Result result = writer.getResult();
  ...
  public void finishBundle(FinishBundleContext c) throws Exception {
    ...
    for (AbstractRowWriter<ElementT> writer : writers.values()) {
      try {
        writer.close();
      } catch (Exception e) {
        exceptionList.add(e);
    ...
    for (Map.Entry<DestinationT, AbstractRowWriter<ElementT>> entry : writers.entrySet()) {
      try {
        DestinationT destination = entry.getKey();
        AbstractRowWriter<ElementT> writer = entry.getValue();
        AbstractRowWriter.Result result = writer.getResult();
        c.output(
